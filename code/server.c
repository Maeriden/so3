...
// 		if(recv_count == 0)
// 		{
// 			// PRINT_ERROR("platform_recv() read 0 bytes");
// 			memory_free(u8, buffer, buffer_size);
// 			return HTTP_STATUS_BAD_REQUEST;
// 		}
...
// 					// Perform a final allocation now that the message size is known
// 					u32 total_size = nontent_len + content_len;
// 					if(total_size > buffer_size)
// 					{
// 						u32 new_buffer_size = total_size;
// 						u8* new_buffer      = memory_realloc(u8, buffer, buffer_size, new_buffer_size);
// 						if(!new_buffer)
// 						{
// 							memory_free(u8, buffer, buffer_size);
